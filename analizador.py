...
# --- SELECTOR DE MODELO A PRUEBA DE ERRORES ---
@st.cache_resource
def cargar_modelo():
    # Intentamos el 1.5 Flash (El mejor)
    try:
        m = genai.GenerativeModel('gemini-1.5-flash')
        m.generate_content("test", generation_config={"max_output_tokens": 1})
        return m
    except:
        # Si falla, intentamos el 1.0 Pro (El más compatible del mundo)
        try:
...
# --- CSS DEFINITIVO (Botones Gemelos y Colores Blindados) ---
st.markdown("""
    <style>
    ...
    .report-card { 
        ...
        margin-bottom: 25px; line-height: 1.6; color: #2c3e50 !important;
    }
...
if model is None:
    st.error("No se pudo conectar con los modelos de Google. Por favor, revisa tu API Key.")
else:
        ...
        if st.button("🚀 ¡DAME LUZ SOBRE MI FACTURA!", type="primary"):
            with st.spinner('ReciboZen está analizando...'):
                try:
                    ...
                    1. Informe detallado para leer: Total (€), Consumo (kWh), Potencia (kW), Impuestos y Consejo.

...
def obtener_modelo_seguro():
    # Intentamos los nombres más comunes en orden de estabilidad
    nombres_a_probar = ['models/gemini-1.5-flash', 'gemini-1.5-flash', 'models/gemini-2.5-flash']
    for nombre in nombres_a_probar:
        try:
            ...
            continue
    # Si fallan los fijos, buscamos dinámicamente
    try:
...
# --- CSS DEFINITIVO (CORRECCIÓN DE UPLOAD Y COLORES) ---
st.markdown("""
    <style>
    ...
    /* ARREGLO DEL CARGADOR DE ARCHIVOS (UPLOAD) */
    ...
    /* Arregla el color de los textos dentro del cargador */
    [data-testid="stFileUploader"] section div div {
        color: #2c3e50 !important;
    }

    /* Arregla el bloque negro de los archivos ya subidos */
...
if model is None:
    st.error("No se pudo conectar con la IA. Verifica los Secrets de Streamlit.")
else:
        ...
        if st.button("🚀 ANALIZAR MI FACTURA", type="primary"):
            with st.spinner('Analizando...'):
                try:
                    ...
                    prompt = f"Resume esta factura de energía para una persona mayor. Indica Total (€), Consumo (kWh) y Potencia. Sé amable. Texto: {texto_factura[:3000]}"

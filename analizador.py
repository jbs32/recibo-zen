...
.feature-item {
    ...
    border: 1px solid rgba(22,119,200,.08);
    border-radius: 18px;
    padding: .9rem 1rem;
...
.feature-dot {
    ...
    box-shadow: 0 0 0 4px rgba(75,180,255,.13);
}
...
.audio-box { background: #f7fbff; border: 1px solid var(--line); border-radius: 18px; padding: .9rem; }
...
.advice-box {
    position: relative;
    background: linear-gradient(135deg, #fff9e8 0%, #fff2c8 100%);
    border: 1px solid rgba(255,183,3,.25);
    border-radius: 18px;
    padding: 1rem 1rem 1rem 1.1rem;
    box-shadow: 0 10px 24px rgba(255,183,3,.12);
}
.advice-box::before {
    content: '⚡';
    position: absolute;
...
.info-panel {
    ...
    border: 1px solid rgba(22,119,200,.1);
    border-radius: 18px;
    padding: .98rem 1rem;
}
.info-panel h3, .advice-box h3 {
    margin: 0 0 .6rem 0;
    font-family: 'Manrope', system-ui, sans-serif !important;
    font-size: 1.06rem;
...
div[data-testid="stFileUploader"] section {
    ...
    border: 2px dashed rgba(22,119,200,.26) !important;
...
def short_period_label(periodo, timestamp):
    ...
    names = {1:["enero"],2:["febrero"],3:["marzo"],4:["abril"],5:["mayo"],6:["junio"],7:["julio"],8:["agosto"],9:["septiembre","setiembre"],10:["octubre"],11:["noviembre"],12:["diciembre"]}
    for n, words in names.items():
        if any(w in lower for w in words):
            year = re.search(r"20\d{2}", lower)
            return f"{MONTHS.get(n)} {year.group()[-2:] if year else ''}".strip()
